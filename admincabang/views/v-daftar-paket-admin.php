        ...
        <div class="panel-toolbar text-right">
          <div class="col-md-11">
           <div class="col-sm-4">
             ...
             <?php if (!empty($post)): ?>
               <input name="filter_sekolah" type="hidden" value="<?=$post['select_sekolah']  ?>">
               <input name="filter_to" type="hidden" value="<?=$post['to']  ?>">
               <input name="filter_paket" type="hidden" value="<?=$post['paket']  ?>">
             <?php endif ?>
             <!-- kalo gada yang di filter -->
             <select class="form-control" id="select_sekolah">
              <option value="all">Semua Sekolah</option>
              <?php foreach ($sekolah as $item): ?>
                <?php $nama_besar = strtoupper($item->namaSekolah) ?>
                <option value="<?=$item->id ?>"><?=$nama_besar?></option>
              <?php endforeach ?>
            </select>
          </div>
...
function prevPage() {
  ...
  $(document).ready(function(){
    ...
    mySelect = $('select[name=select_sekolah]').val();
